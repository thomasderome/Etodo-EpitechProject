...
async function create_share(data) {
    const [verif] = await pool.query("SELECT * FROM todo WHERE id=? AND user_id=?", [data.todo_id, data.user_id]);
    ...
    const [user] = await pool.query("SELECT user.email, user.name, user.firstname FROM shared_todo JOIN user ON user.id=shared_todo.user_id WHERE shared_todo.id = ?", [id.insertId]);
    return user[0]
}

async function get_setting(user_id, todo_list_id) {
    const verif = pool.query("SELECT id FROM todo WHERE user_id=? AND id=?", [user_id, todo_list_id]);
    if (!verif[0]) return null;

    const [result] = pool.query("SELECT * FROM shared_todo WHERE todo_list_id=?", [todo_list_id]);
    return result
}

async function get_share(user_id) {
    const [result] = await pool.query("SELECT shared_todo.*, todo.title, todo.status FROM shared_todo JOIN todo ON todo.id=shared_todo.todo_list_id WHERE shared_todo.user_id=?", [user_id]);
    return result;
}
...
async function get_user_in_share(todo_list_id, user_id) {
    ...
    const [owner] = await pool.query("SELECT user_id FROM todo WHERE id=?", [todo_list_id]);
...
async function remove_share(data) {
    const [verif] = await pool.query("SELECT * FROM shared_todo WHERE user_id=? AND id=?", [data.user_id, data.todo_list_id]);
    if (!verif[0]) return null;

    await pool.query("DELETE FROM shared_todo WHERE user_id=?", [data.user_remove_share]);
    return true;
}

module.exports = {create_share, get_share, get_user_in_share, remove_share, get_setting};
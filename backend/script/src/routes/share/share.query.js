...
async function create_share(data) {
    const [verif] = await pool.query("SELECT id FROM todo WHERE id=? AND user_id=?", [data.todo_id, data.user_id]);
...
async function get_setting(user_id, todo_list_id) {
    const verif = await pool.query("SELECT id FROM todo WHERE user_id=? AND id=?", [user_id, todo_list_id]);
    if (!verif[0]) return null;
...
async function change_mode(share_id, user_id) {
    const [verif] = await pool.query("SELECT * FROM shared_todo JOIN todo ON todo.id=shared_todo.todo_list_id WHERE todo.user_id=? AND shared_todo.id", [user_id, share_id]);
    if (!verif[0]) return null;
...
async function remove_share(data) {
    const [verif] = await pool.query("SELECT * FROM shared_todo JOIN todo ON todo.id=shared_todo.todo_list_id WHERE todo.user_id=? AND shared_todo.id", [data.user_id, data.share_id]);
    if (!verif[0]) return null;
